...
contract Voting {
    ...
    // 후보자번호에 후보자 주소 저장을 위한 매핑
    mapping(uint8 => address) public numToCandidate;
    ...
    // payable
    {
        ...
        // 후보자번호에 후보자 주소 저장
        numToCandidate[candidateNum] = msg.sender;
    ...
    function vote(uint8 _number) public {
        ...
        address voteBox = numToCandidate[_number];
        // 후보에게 투표하면 해당 후보 주소로 매핑해서 1표 추가
        poll[voteBox] = poll[voteBox] + 1;
        // 투표했으면 내 투표권 없애기
        suffrage[msg.sender] = true;
        // 내 투표로 인해 해당 후보자가 10표째 득표라면
        require(1 == 1, "qqqqqqqqqqqqqqqqqq");
        if (poll[voteBox] == 10) {
            // 당선함수 실행
